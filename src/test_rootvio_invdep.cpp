...
int main() {
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        // 为初值添加随机噪声
        // 计算特征点在首帧观测中的逆深度
        rootVIO::Vector3<Scalar> pc = cameras[0].q_wb.inverse() * (points[i] - cameras[0].t_wb);
        std::normal_distribution<double> landmark_position_noise(0., 0.5);
        Scalar noise(landmark_position_noise(generator));
    ...
    for (size_t i = 1; i < cameras.size(); ++i) {
        ...
        order.BA = 9;
        order.BG = 12;
        // rootVIO::Matrix15<Scalar> temp = rootVIO::Matrix15<Scalar>::Random(15, 15) * 1e-4;
        // cov = temp + temp.transpose();
        // cov += rootVIO::Matrix15<Scalar>::Identity(15, 15);
        cov.setIdentity();
        cov = cov * 1e-4;
    ...
    // problem.SetThreshold(1e-6, 2, 1e-6, 2); // 设置终止迭代判断阈值
    ...
    problem.Solve(10);     // 求解问题，设置最大迭代步数
    // problem.Test();
